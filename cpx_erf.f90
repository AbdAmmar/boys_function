...
!
!                       error function for complex-valued arguments z
!
!    \erf(z) = \frac{2}{\sqrt{\pi}} \int_0^{z} \exp(-t^2) dt, with z = x + \imath y
! 
! algorithmic steps by Marcel Leutenegger:
...
complex*16 function cpx_erf(x, y)
  ...
  double precision, external   :: erf_F 
  complex*16,       external   :: erf_E, erf_G, erf_H
...
complex*16 function erf_E(x, yabs)
  ...
  double precision             :: inv_pi = 1.d0 / dacos(-1.d0)
...
complex*16 function erf_G(x, yabs)
  ...
  double precision             :: dlog_2pi = dlog(2.d0 * dacos(-1.d0))
...
complex*16 function erf_H(x, yabs)
  ...
  if( (dabs(x) .lt. 10.d0) .and. (yabs .lt. 6.1d0) ) then
